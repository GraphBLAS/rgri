...
template <typename Iterator, typename Fn>
using transform_vector_iterator = grb::detail::iterator_adaptor<transform_vector_accessor<Iterator, Fn>>;

template <grb::VectorRange VectorType,
          std::copy_constructible Fn>
class transform_vector_view : public std::ranges::view_interface<transform_vector_view<VectorType, Fn>>
{
public:
  using vector_type = std::decay_t<VectorType>;

  using index_type = container_index_t<vector_type>;
  using scalar_type = decltype(std::declval<Fn>()(std::declval<container_value_t<vector_type>>()));
  ...
  using iterator = transform_vector_iterator<decltype(std::declval<std::ranges::views::all_t<VectorType>>().base().begin()), Fn>;
  using const_iterator = iterator;

  using value_type = std::remove_cvref_t<decltype(*std::declval<iterator>())>;

  transform_vector_view(VectorType&& vector, Fn fn) : vector_(vector), fn_(fn) {}
...
template <grb::VectorRange VectorType,
          std::copy_constructible Fn>
transform_vector_view(VectorType&&, Fn) -> transform_vector_view<VectorType, Fn>;
