...
inline constexpr bool has_identity_template_v = requires {
  { Fn::template identity<T>() } -> std::same_as<T>;
};

template <typename Fn, typename T>
inline constexpr bool has_identity_method_v = requires {
  { Fn::identity() } -> std::same_as<T>;
};

template <typename Fn, typename T>
  requires(is_binary_op_v<Fn, T, T, T> &&
...
inline constexpr bool has_identity_v = requires {
  { grb::monoid_traits<Fn, T>::identity() } -> std::same_as<T>;
};

template <typename Fn, typename T>
inline constexpr bool is_monoid_v =
    is_binary_op_v<Fn, T, T, T> && has_identity_v<Fn, T>;
...
template <typename Fn, typename T>
concept Monoid = BinaryOperator<Fn, T, T, T> && requires {
  { grb::monoid_traits<Fn, T>::identity() } -> std::same_as<T>;
};
