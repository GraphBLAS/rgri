...
TEMPLATE_TEST_CASE( "basic operator test plus 1", "[template]",
...
TEMPLATE_TEST_CASE( "basic operator test minus 1", "[template]",
          int, size_t, float, double, bool ) {
	using T = TestType;
	std::size_t num_values = 1000;
...
TEMPLATE_TEST_CASE( "basic operator test multiplies 1", "[template]",
          int, size_t, float, double, bool ) {
	using T = TestType;
	std::size_t num_values = 1000;
	...
	test_op<true>(num_values, T(0), grb::multiplies<T, T>(), std::multiplies<T>());
	test_op<true>(num_values, T(0), grb::multiplies<T, T, T>(), std::multiplies<T>());
}
